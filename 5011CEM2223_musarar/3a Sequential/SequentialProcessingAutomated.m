function [tSeq] = SequentialProcessingAutomated(LoopParameter)

%% This script allows you to open and explore the data in a *.nc file

close all

FileName = 'C:\Users\ruten\OneDrive\Desktop\5011 Big Data Programming\Model\Model\o3_surface_20180701000000.nc';
...
for NumHour = 1:25 % loop through each hour
    %NumHour = 1:25; %loop through each hour
    ...
    for idx = 1:5000; size(Data2Process,1) % step through each data location to process the data
      ...
      %% Automated Testing
      DataOptions = [250,5000,9000]; %defining the loop variable
      
      for idx = 1:size(DataOptions, 1)
          LoopParameter = DataOptions(idx);
          %process
          Results(idx, :) = [DataOptions(idx), RunTime];
      end
    end
